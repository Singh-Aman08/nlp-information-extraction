...
The exploratory clustering analysis provided preliminary evidence that sentence embeddings capture meaningful semantic structure within clinical trial abstracts. Using sentence embeddings generated by **SentenceTransformer (all-MiniLM-L6-v2)** and K-means clustering, the optimal number of clusters was found to be three, aligning with the target extraction categories: Participants (PART), Intervention (INT), and Outcomes (OUT). This suggests that semantically similar clinical statements naturally group together and supports the suitability of embedding-based representations for downstream information extraction tasks.

The experimental results demonstrate a clear performance hierarchy across the different information extraction strategies. Large Language Model-based approaches consistently outperform traditional methods, with the LoRA fine-tuned model achieving the best overall performance across both micro-averaged and macro-averaged evaluations. This indicates that domain adaptation through fine-tuning significantly improves the model’s ability to extract structured biomedical information.
...
Field-level evaluation reveals that Intervention (INT) and Outcome (OUT) entities are generally extracted more accurately than Participant (PART) entities. Although performance improves substantially with LLM-based methods, the PART category remains the most challenging across all approaches. This is likely due to the variability and complexity of participant descriptions in clinical trial abstracts, which often contain detailed demographic and eligibility information.

Overall, the results indicate that contextual language understanding plays a critical role in structured biomedical information extraction. The progression from rule-based systems to CRF models and ultimately to fine-tuned LLMs demonstrates the increasing importance of semantic and contextual representations for accurately extracting biomedical information from unstructured clinical text.
